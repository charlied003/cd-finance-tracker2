        ...
        {transactions.length===0 ? (
          ...
          <>
            ...
            {view==="spend" && <SpendView
              ...
              allTransactions={applyRules(transactions)} />}
...
function detectSubscriptions(allTxns) {
  ...
  // Group by merchant
  const byDesc = {};
  spending.forEach(t => { const k = t.description || "Unknown"; if (!byDesc[k]) byDesc[k] = []; byDesc[k].push(t); });

  const subs = [];

  Object.entries(byDesc).forEach(([name, txns]) => {
    if (txns.length < 2) return;

    // Find the largest cluster of similarly-priced transactions.
    // Use each transaction's amount as a candidate cluster centre and
    // collect all transactions within 15% of it. Keep the biggest cluster.
    // This lets us ignore one-off or differently-priced charges from the same merchant.
    let bestCluster = [];
    ...
    // Sort cluster by date and calculate day-gaps between consecutive payments
    ...
    // All gaps must be within 30% of the average (handles Feb, weekends etc.)
    ...
    subs.push({
      name, amount: avgAmount, frequency,
      lastPaid: last.date, daysSince,
      nextDue: nextDate.toISOString().slice(0, 10), daysUntil,
      category: last.category || "Other", count: sorted.length,
    });
  });
        ...
        <div style={{marginBottom:16}}>
          ...
          {showSubs&&(
            <div style={{background:"#0f1117",border:"1px solid #a78bfa30",borderRadius:10,overflow:"hidden"}}>
              {subs.map((s,i)=>{
                const overdue  = s.daysUntil < 0;
                const soon     = s.daysUntil >= 0 && s.daysUntil <= 5;
                const dotColor = overdue ? "#f87171" : soon ? "#fbbf24" : "#a78bfa";
                const dueLabel = overdue
                  ? `Overdue by ${Math.abs(s.daysUntil)} day${Math.abs(s.daysUntil)===1?"":"s"}`
                ...
                return (
                  <div key={s.name} style={{padding:"10px 14px",borderBottom:i<subs.length-1?"1px solid #1c1f2e":"none"}}>
                    <div style={{display:"flex",alignItems:"center",gap:8}}>
                      <div style={{width:7,height:7,borderRadius:"50%",background:dotColor,flexShrink:0}}/>
                      <span style={{fontSize:13,fontWeight:700,flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.name}</span>
                      <span style={{fontSize:11,color:"#4b5563",marginRight:8}}>{s.frequency}</span>
                      <span style={{fontSize:14,fontWeight:700,color:"#a78bfa"}}>{fmt(s.amount)}</span>
                    </div>
