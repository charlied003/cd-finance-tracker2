...
function detectSubscriptions(allTxns) {
  ...
  Object.entries(byDesc).forEach(([name, txns]) => {
    ...
    if (!amounts.every(a => Math.abs(a - avg) / avg < 0.10)) return;
    const intervals = [];
    ...
    subs.push({ name, amount: avg, frequency, lastPaid: last.date, daysSince, nextDue, daysUntil, category: last.category || "Other", count: sorted.length });
  });
